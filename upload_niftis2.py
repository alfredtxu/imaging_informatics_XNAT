...
REMOTE_END= "?overwrite=delete&format=NIFTI&content=NIFTI_RAW"""
...
def main(argv):  
    ...
    """Upload NIFTIS that have been converted but not imported into XNAT due to REST errors. Shortened version of mutli_sort_archivev3.     """
    svr.configure_server()   
    archive = svr.xnat_archive_path
    zip_path = svr.seven_zip_path
    parsar = argparse.ArgumentParser(description='Fix missed NIftis: Upload missed NIFTIS and list those requiring dcm2nii')
   
    parsar.add_argument('-js',  default=' -u import -p tropmi', help='Jsession, optional. "create" to generate a new jsession')
    args = parsar.parse_args()
    jsession = args.js
    
    print jsession
    
    if 'create' in jsession:
        jsession = svr.get_jsession()
        print 'Creating jsession:', jsession
    elif '-u import' not in jsession:
        jsession = ' -user_session ' + jsession + ' '
    ...
    fc = svr.make_file('','upload_niftis_to_covert', 'log')  
    fr = svr.make_file('','upload_niftis_to_upload', 'bat')      
    number_scans = len(s)
    num = 1
    testn = 0
                    ...
                    for nifti_file in nifti_files:
                        if 'nii.gz' in nifti_file:
                            try:
                                ...
                                upload_file  = svr.xnat_rest + "  -host "+ svr.xnat_host + jsession +" -m PUT  -remote " + REMOTE + local
                                #upload_file = svr.xnat_rest(jsession, 'PUT',  REMOTE + local)  
                                print upload_file 
                                fr.write(upload_file + '\n')
